...
class HandboxDecoder(Node):
    ...
    def tensor_callback(self, msg: TensorList):
        ...
        self.get_logger().info(f"{best_pred.shape}")
        ...
        if not valid:
            ...
            self._use_last_roi()
            return
        self.get_logger().info(f"{best_pred}")

        x1_net, y1_net, x2_net, y2_net = best_pred[0], best_pred[1], best_pred[2], best_pred[3]
        #self.get_logger().info(f"{x1_net} {y1_net} {x2_net} {y2_net}")
        ...
        margin_x = 0 #0.08 * float(src_w)
        margin_y = 0 #0.08 * float(src_h)
